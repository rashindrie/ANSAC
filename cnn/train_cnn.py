...
def main(model_name, result_dir, configs, summary_csv):
    ...
    if configs['mixed_model']:
        train_dataset = MixedDataset(img_dir=configs['feature_dir'], weights_dir=configs['weight_dir'],
                                     csv_file=train_csv, train=True, label_column='label', extension=configs['ext'])
        val_dataset = MixedDataset(img_dir=configs['feature_dir'], weights_dir=configs['weight_dir'],
                                   csv_file=val_csv, train=False, label_column='label', extension=configs['ext'])
        test_dataset = MixedDataset(img_dir=configs['feature_dir'], weights_dir=configs['weight_dir'],
                                    csv_file=test_csv, train=False, label_column='label', extension=configs['ext'])
    else:
        train_dataset = IndividualDataset(img_dir=configs['feature_dir'], weights_dir=configs['weight_dir'],
                                          csv_file=train_csv, train=True, label_column='label',
                                          extension=configs['ext'], is_tcga=is_tcga)
        val_dataset = IndividualDataset(img_dir=configs['feature_dir'], weights_dir=configs['weight_dir'],
                                        csv_file=val_csv, train=False, label_column='label', extension=configs['ext'],
                                        is_tcga=is_tcga)
        test_dataset = IndividualDataset(img_dir=configs['feature_dir'], weights_dir=configs['weight_dir'],
                                         csv_file=test_csv, train=False, label_column='label', extension=configs['ext'],
                                         is_tcga=is_tcga)
    ...
    model = CNN(num_classes=num_classes, **configs['model'])
    test_model = CNN(num_classes=num_classes, **configs['model'])
    ...
    test_loss, test_sensitivity, test_specificity, test_fscore, test_weighted_accuracy, test_f1, auc, fpr, tpr, thresholds = test_from_checkpoint(
        ...
        criterion=criterion, configs=configs, device=device, result_dir=result_dir,
        summary_csv=summary_csv, classes=unique_classes)

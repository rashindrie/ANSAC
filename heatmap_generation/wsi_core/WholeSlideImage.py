...
#             w, h = region_size
#             print(region_size)
            region_size_orig = self.level_dim[vis_level]
            region_size = (20480, 20480)
            top_left = (0,0)
            bot_right = (20480, 20480)
            w, h = region_size
        ...
        scores /= 100
        print(np.max(scores), np.min(scores), scores)
...
#             plt.show()
        ...
        if not blank_canvas:
            ...
            img = self.wsi.read_region(top_left, vis_level, region_size_orig).convert("RGB")
            img = img.resize(region_size)
            orig_img = img
            img = np.array(img)
        else:
            # use blank canvas
        ...
        if alpha < 1.0:
            img = self.block_blending(img, orig_img, vis_level, top_left, bot_right, alpha=alpha, blank_canvas=blank_canvas, block_size=1024)
    ...
    def block_blending(self, img, orig_img, vis_level, top_left, bot_right, alpha=0.5, blank_canvas=False, block_size=1024):
        print('\ncomputing blend')
        downsample = self.level_downsamples[vis_level]
        ...
        for x_start in range(top_left[0], bot_right[0], block_size_x * int(downsample[0])):
            for y_start in range(top_left[1], bot_right[1], block_size_y * int(downsample[1])):
                ...
                if not blank_canvas:
                    ...
                    pt = (x_start, y_start)
                    canvas = orig_img[y_start:y_start+blend_block_size[1], x_start:x_start+blend_block_size[0]]

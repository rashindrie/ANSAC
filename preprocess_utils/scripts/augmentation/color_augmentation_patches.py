...
from preprocess_utils.scripts.utils import (
    convert_image_to_matrix, convert_matrix_to_image)
from preprocess_utils.scripts.color_conversion import (
    rgb_to_sda, sda_to_rgb)
from preprocess_utils.scripts.color_deconvolution import (
    color_deconvolution_routine)


def perturb_stain_concentration(
        StainsFloat, W, I_0=None, mask_out=None, alpha=None, beta=None):
    ...
    for i in range(3):
        augmented_sda[i, keep_mask[i, :]] *= alpha[i]
        augmented_sda[i, keep_mask[i, :]] += beta[i]
...
class StainAugmentationPatches(object):
    def __init__(self, sigma1=0.9, sigma2=0.9):
        self.alpha = []
        self.beta = []

        for i in range(3):
            self.alpha.append(np.random.uniform(1 - sigma1, 1 + sigma1))
            self.beta.append(np.random.uniform(-sigma2, sigma2))

    def __call__(self, img):
        image = np.array(img)
        aug_image = rgb_perturb_stain_concentration(image, alpha=self.alpha, beta=self.beta)
        return Image.fromarray(aug_image.astype('uint8'), 'RGB')
